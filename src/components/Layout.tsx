    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-brand-bg/90 backdrop-blur-md py-3 md:py-4 shadow-sm' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center">
        <Link to="/" className="flex items-center group">
          <img src={Logo} alt="Studio Mahen'i" className="h-14 md:h-18 object-contain group-hover:scale-105 transition-transform duration-500 [&>*]:fill-brand-primary" style={{ filter: "brightness(0) saturate(100%) invert(23%) sepia(24%) saturate(1112%) hue-rotate(347deg) brightness(95%) contrast(88%)" }} />
        </Link>
  ...
  <footer className="py-12 bg-brand-primary text-brand-bg/40 border-t border-brand-bg/5 mt-auto">
    <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
      ...
      <div className="text-sm font-medium">
        &copy; {new Date().getFullYear()} Studio <b>Maheni</b>. Tous droits réservés.
      </div>

      <div className="flex gap-8 text-xs font-bold uppercase tracking-widest">
        <a href="#" className="hover:text-brand-bg transition-colors" aria-label="Mentions Légales">Mentions Légales</a>
        <a href="#" className="hover:text-brand-bg transition-colors" aria-label="Politique de Confidentialité">Confidentialité</a>
      </div>
    </div>
  </footer>
);
